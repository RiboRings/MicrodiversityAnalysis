...
df <- mags %>%
  filter(RpkmMean > rpkm_threshold) %>%
  arrange(desc(NuclDivMean))

mat <- df %>%
  transmute(S1 = nucl_diversity_1,
         S2 = nucl_diversity_2,
         S3 = nucl_diversity_3,
         S4 = nucl_diversity_4,
         S5 = nucl_diversity_5,
...
Heatmap(mat,
#        heatmap_legend_param = list(at = seq(0, max(mat), by = 10)),
...
```{r multipanel_2, out.width = "125%", out.height = "125%", include = FALSE}
grid.arrange(
  arrangeGrob(grobs = lapply(plots, function(p) p + guides(colour = FALSE)), 
              ncol = 3, 
              bottom = textGrob("RPKM", gp = gpar(fontface = "bold",
                                                  col = "red",
                                                  fontsize = 15)), 
              left = textGrob("Nucleotide Diversity / Divergent Sites per Mbp",
                              gp = gpar(fontface = "bold",
                                        col="blue",
                                        fontsize = 12),
                              rot = 90)),
...
for (i in 1:9) {
  ...
  plots[[i]] <- mags %>%
  mutate(Y = eval((parse(text = paste0("DiSiperMbp_", i)))),
         X = eval((parse(text = paste0("nucl_diversity_", i)))),
         colour = eval(parse(text = paste0("RPKM_", i)))) %>%
  filter(colour > rpkm_threshold) %>%
  ggplot(aes(x = X, y = Y, colour = colour)) +
    geom_point(size = 1) +
...
```{r multipanel_3, out.width = "125%", out.height = "125%", include = FALSE}
grid.arrange(
  arrangeGrob(grobs = lapply(plots, function(p) p + guides(colour = FALSE)), 
              ncol = 3, 
              bottom = textGrob("Nucleotide Diversity",
                                gp = gpar(fontface = "bold",
                                          col = "red",
                                          fontsize = 15)), 
              left = textGrob("Divergent Sites per Mbp",
                              gp = gpar(fontface = "bold",
                                        col="blue",
                                        fontsize = 15),
                              rot = 90)),
